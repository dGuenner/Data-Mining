...
enhanceData <- function(patients) {
  ...
  for (i in 1:nrow(patients)) {
    ...
    add_duration_to_phases <- function(startAge, endAge) {
    ...
    currentStartCols <- grep("^startAge_", names(patients), value = TRUE)
    for (col in currentStartCols) {
      startAge <- patients[[col]][i]
      endAge <- current_age
      add_duration_to_phases(startAge, endAge)
    }
...
cat(glue::glue("Durchschnittliche Einnahmedauer von aktiven Prophylaxe-Meds (in Jahren):\n  
