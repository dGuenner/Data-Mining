...
chisq_test_medication <- function(patients) {
    ...
    # Mann-Whitney-U Tests für paarweise Vergleiche zwischen Medikationsgruppen
    print("Mann-Whitney-U Tests für Altersunterschiede zwischen Medikationsgruppen:")

    # Zuerst etwas Diagnostik ausgeben
    print("Diagnostik für Altersgruppenvariable:")
    print(table(filtered_patients$menopause))
    print(class(filtered_patients$menopause))

    # Stellen Sie sicher, dass die Altersgruppenvariable numerisch ist
    # Umwandlung von Faktoren/Zeichenketten (falls vorhanden) in numerische Werte
    filtered_patients$ageGroup10_numeric <- as.numeric(filtered_patients$ageGroup10)

    # Erstelle eine Liste für alle Medikationsgruppen mit numerischen Alterswerten
    med_groups_list <- list()
    for (group in unique(med_groups_vector)) {
        values <- filtered_patients$ageGroup10_numeric[med_groups_vector == group]
        med_groups_list[[group]] <- values
        print(paste(
            "Gruppe:", group, "- Anzahl Datenpunkte:", length(values),
            "- Gültige Werte:", sum(!is.na(values))
        ))
    }

    # Führe paarweise Tests durch
    results <- list()
    for (i in 1:(length(unique(names(med_groups_list))) - 1)) {
        for (j in (i + 1):length(unique(names(med_groups_list)))) {
            group1 <- unique(names(med_groups_list))[i]
            group2 <- unique(names(med_groups_list))[j]

            # Überprüfe die Anzahl der gültigen Daten
            valid_data1 <- med_groups_list[[group1]][!is.na(med_groups_list[[group1]])]
            valid_data2 <- med_groups_list[[group2]][!is.na(med_groups_list[[group2]])]

            if (length(valid_data1) >= 3 && length(valid_data2) >= 3) {
                # Führe den Test durch
                tryCatch(
                    {
                        test_result <- wilcox.test(valid_data1, valid_data2)
                        ...
                        # Berechne mediane Werte für bessere Interpretation
                        median1 <- median(valid_data1, na.rm = TRUE)
                        median2 <- median(valid_data2, na.rm = TRUE)

                        # Berechne durchschnittlichen Rang für jede Gruppe
                        all_data <- c(valid_data1, valid_data2)
                        all_ranks <- rank(all_data)
                        mean_rank1 <- mean(all_ranks[1:length(valid_data1)])
                        mean_rank2 <- mean(all_ranks[(length(valid_data1) + 1):length(all_ranks)])

                        # Printe Ergebnisse mit Interpretation
                        direction <- ifelse(mean_rank1 > mean_rank2,
                            paste(group1, "tendenziell bei älteren Patientinnen"),
                            paste(group2, "tendenziell bei älteren Patientinnen")
                        )

                        print(paste(
                            group1, "(n=", length(valid_data1), ", median=", round(median1, 1),
                            ") vs ", group2, "(n=", length(valid_data2), ", median=",
                            round(median2, 1), "): p =", round(test_result$p.value, 4),
                            "-", direction
                        ))
                    },
                    error = function(e) {
                        print(paste("Fehler beim Testen von", group1, "vs", group2, ":", e$message))
                    }
                )
            } else {
                print(paste(
                    "Überspringe", group1, "vs", group2, "- Zu wenige gültige Daten:",
                    "Gruppe 1:", length(valid_data1), "Werte,",
                    "Gruppe 2:", length(valid_data2), "Werte"
                ))
            }
        }
    }

    # Nur wenn results nicht leer ist
    if (length(results) > 0) {
        # Bonferroni-Korrektur für multiple Tests
        alpha_corrected <- 0.05 / length(results)
        print(paste("Bonferroni-korrigiertes Signifikanzniveau:", alpha_corrected))
    } else {
        print("Keine gültigen Tests durchgeführt - kein korrigiertes Alpha berechnet.")
    }
}
